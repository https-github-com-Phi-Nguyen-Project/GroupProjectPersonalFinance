...
class DBOpenHelper (context: Context):
    ...
    fun getTransaction(id: Int): Items {
        ...
        return Items(id, value, title, isExpense, date, desc, cat)
    }

    fun updateTransaction(items: Items) {

        val whereClause = "${BaseColumns._ID} = ?"
        val expenseFlag = if(items.isExpense) 1 else 0
        val dateFormat = SimpleDateFormat("yyyy-MM-dd HH:mm:ss")

        val newTransaction = ContentValues().apply {
            put(DBContract.TransactionEntry.COLUMN_VALUE, items.value)
            put(DBContract.TransactionEntry.COLUMN_IS_EXPENSE, expenseFlag)
            put(DBContract.TransactionEntry.COLUMN_DATE, dateFormat.format(items.date))
            put(DBContract.TransactionEntry.COLUMN_TITLE, items.title)
            put(DBContract.TransactionEntry.COLUMN_CATEGORY, items.category)
            put(DBContract.TransactionEntry.COLUMN_DESCRIPTION, items.desc)
        }

        val db = this.writableDatabase
        val rowsUpdated = db.update("${DBContract.TransactionEntry.TABLE_NAME}", newTransaction, whereClause, arrayOf(items.id.toString()))
